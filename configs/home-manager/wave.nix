...
let
  backgrounds = pkgs.writeText "wave-backgrounds.json" (
    builtins.toJSON {
      "bg@default" = {
  ...
  themes = pkgs.writeText "wave-themes.json" (
    builtins.toJSON {
      "default-dark" = {
  ...
  settings = pkgs.writeText "wave-settings.json" (
    builtins.toJSON {
...
in
{
  home.activation.waveSettings = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
    ...
    if [ ! -e "$target"/backgrounds.json ]; then
      install -D --mode=644 --owner=${config.home.username} --group=users ${backgrounds} "$target"/backgrounds.json
    fi
    if [ ! -e "$target"/termthemes.json ]; then
      install -D --mode=644 --owner=${config.home.username} --group=users ${themes} "$target"/termthemes.json
    fi
