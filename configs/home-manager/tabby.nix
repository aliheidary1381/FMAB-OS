...
let
  ...
  tabbyConfig = yamlFormat.generate "tabby-config.yaml" {
    version = 8;
    profiles = [ ];
    groups = [ ];
    ...
    terminal = {
      ...
      ligatures = true;
      profile = "local:runcurrent-systemswbinfish";
      wordSeparator = " ()[]{}'\"_";
      customColorSchemes = [ ];
...
in
{
  home.activation.tabbyConfig = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
    ...
    if [ ! -e "$target" ]; then
      install -D --mode=644 --owner=${config.home.username} --group=users ${tabbyConfig} "$target"
    fi
  '';
}
