...
function where
  ...
  nix-store --query --outputs $store_derivation # nix path-info nixpkgs#"$argv[1]"
end
...
function icon
    ...
    for file in (find . -type f \( -iname "folder.jpg" -o -iname "folder.jpeg" -o -iname "folder.png" \
        ...
        set ext (string lower (string split -r -m1 . "$base")[2])
        set abs_dir (realpath "$dir")
        set abs_file (realpath "$file")
        set target_png "$abs_dir/folder.png"

        if not test -f "$target_png"
            if test "$ext" != "png"
                magick "$abs_file" "$target_png"
            else
                if test "$base" != "folder.png"
                    mv "$abs_file" "$target_png"
                end
            end
        end

        echo "[Desktop Entry]
Icon=$target_png" > "$abs_dir/.directory"
    end
end
